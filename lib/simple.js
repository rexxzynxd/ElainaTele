...
const conn = new Telegraf('7207786423:AAFGAw-9QTEPSXflgKZIWh1vTCQfwZAp00s');
...
// ======================================= FUNCTION ================================== //
    ...
    conn.sendAudio = async (jid, urlOrBuffer, caption, quoted, options = {}) => {
      const link = /^https?:\/\//i.test(urlOrBuffer) ? urlOrBuffer : undefined;
      if (link) {
        await conn.telegram.sendAudio(jid, link, {
          ...options,
          caption,
          reply_to_message_id: quoted,
        });
      } else {
        await conn.telegram.sendAudio(jid, { source: Buffer.from(urlOrBuffer) }, {
          ...options,
          caption,
          reply_to_message_id: quoted,
        });
      ...
      conn.use(async (ctx, next) => {
        if (ctx.update.message && ctx.update.message.text) {
            ...
            if (matchingButton) {
                ...
                const [command, ...args] = callbackData.replace(/^\//, '').split(/\s+/);
                    ...
                    try {
                        await loadedCommands[command]({
                            ...
                            m: {
                                ...
                                send: ctx.update.message.message_id
                            },
                            prefix,
                            command,
                ...
                try {
                    await loadedCommands[command]({
                        ...
                        m: {
                            ...
                            send: ctx.update.callback_query.message.message_id
                        },
                        prefix,
                        command,
                        commands: loadedCommands,
              ...
              if (loadedCommands[command]) {
                  try {
                      await loadedCommands[command]({
                          ...
                          m: {
                              chat: ctx.update.callback_query.message.chat.id,
                              send: ctx.update.callback_query.message.reply_to_message.message_id
                          },
                          prefix: prefix,
                          command,
...
export function serialize(m, conn, ctx) {
  return {
    conn,
    chat: m.chat.id,
    send: m.update.message.message_id,
        ...
        try {
            ...
            if (commandModule.cmd && commandModule.run) {
                for (const cmd of commandModule.cmd) {
                    allCommands[cmd] = async ({ conn, m, text, prefix, command, args }) => {
                        await commandModule.run({ conn, m, text, prefix, command, args });
                    };
                }
            } else {
